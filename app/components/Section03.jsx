  ...
  return (
    <div className="grid max-w-[90%] mx-auto mt-25" id="solution">
      <div className="flex flex-col gap-2 items-center px-10">
        {/* ---------------- HEADING ---------------- */}
        <button className="bg-[#B1E0FC40] px-4 py-2 border border-[#B1E0FC] ring-7 ring-[#B1E0FC40] rounded-lg text-[16px]">
          SOLUTIONS
        </button>

        <h1 className="text-4xl font-medium font-clauson mt-3">
          How Lawyers Use Our AI
        </h1>

        {/* ---------------- TABS ---------------- */}
        <div className="bg-[#0D0D0D0D] h-16 w-full mt-5 grid grid-cols-4 gap-px px-3 py-2 border border-solid border-[#12121533] rounded-xl mb-20">
          {TAB_CONTENT.map((item, index) => (
            <div
              key={index}
              onClick={() => handleTabChange(index)}
              className={`cursor-pointer rounded-md p-[1.5px] transition
                ${
                  activeTab === index
        ...
        <div className="grid grid-cols-2 border border-[#12121533] rounded-2xl overflow-hidden">
          ...
          <div className="relative w-210">
            ...
            {!isPlaying && (
              <button
                onClick={handlePlay}
                className="absolute inset-0 m-auto w-16 h-16 rounded-full bg-[#B1E0FC]
